...
COLLECTION_NAME = "autodidact_ai_core"
...
class IntakeAgent:
  ...
  def __init__(self, collection_name: str = COLLECTION_NAME):
    ...
    # NOTE: For simplicity, we are using Chroma's default embedding function (all-MiniLM-L6-v2).
    # When we go to prod, we will configure a custom embedding function
    # (e.g., via the llm_client or a seperate service) and pass it here.

  def _get_or_create_collection(self):
    """Ensures the vector collection exists and returns it."""
    if self._collection is None:
      # get_or_create_collection handles both creation and retrieval
  ...
  def process_and_add_document(self, content: str, source_url: str, metadata: Dict[str, Any]) -> str:
    """
    Simulates the full ingestion pipeline: 
    1. Prepares the document (e.g., simple chunking).
    2. Gets the collection.
    3. Adds the document to the collection, which generates the embedding.
    """
    collection = self._get_or_create_collection()
        
    # --- 1. SIMULATED CHUNKING ---
    ...
    for i, chunk in enumerate(chunks):
      ...
      chunk_id = f"{metadata.get('instrument_id', 'general')}-{str(uuid.uuid4())[:8]}-{i}"
      
      # Combine the input metadata with chunk-specific metadata
      chunk_metadata = {
          "source": source_url,
    ...
    # --- 3. ADD TO CHROMA ---
    # Chroma calculates the embeddings automatically using its default function
    ...
    print("✅ Document successfully ingested and embedded.")
...
if __name__ == "__main__":
  ...
  # 2. Define the core metadata required by your RAG architecture (from Autodidact AI plan)
  # The 'helpfulness_score' will eventually come from the Validation Agent.
  sample_metadata = {
      "instrument_id": "electric_guitar",
      "technique": "sweeping",
      "difficulty": "advanced",
      "helpfulness_score": 0.95  # Simulated value for a great resource
  }
